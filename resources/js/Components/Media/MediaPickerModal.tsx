    ...
    const handleUploaded = async (uploadedIds: number[]) => {
        ...
        try {
            const items = await fetchByIds(
                multiple ? uploadedIds : [uploadedIds[0]],
            );
            onPicked?.(items);
            ...
            onPicked?.(
                (multiple ? uploadedIds : [uploadedIds[0]]).map(
                    (id) => ({ id, kind: 'file' }) as any,
                ),
            );
            onHide();
        }
            ...
            <Modal.Header closeButton>
                <div>
                    ...
                    <div className="small text-muted">
                        {attachOnConfirm ? (
                            <>
                                Attach to <b>{targetType}</b> #{targetId} —
                                collection: <b>{collection}</b>
                                <span className="ms-2">
                                    • Click to use. Ctrl/Cmd+Click to
                                    multi-select.
                                </span>
                            </>
                        ) : (
                            <>
                                Create mode — items will be attached after you
                                save the {targetType}.
                            </>
                        )}
                    </div>
