...
type Props = {
  ...
  onConfirmSelect?: (selectedIds: number[], selectedItems: MediaItem[]) => Promise<void> | void;
...
export default function MediaBankBrowser({
  ...
  const [type, setType] = useState < "all" | "image" | "doc" | "youtube" > (initialType);
  ...
  const load = async (p = 1) => {
    const { data } = await axios.get < Page < MediaItem >> (route("media.index"), {
  ...
  const removeFromQueue = (key: string) => {
    setQueue((prev) => prev.filter((f) => `${f.name}|${f.size}|${f.lastModified}` !== key));
  };
  ...
  const addYoutube = async () => {
    ...
    try {
      ...
      if (selectable && newId) {
        setSelected((prev) => (multiple ? Array.from(new Set([...prev, newId])) : [newId]));
        setSelectedMap((prev) => ({ ...prev, [newId]: prev[newId] ?? ({ id: newId } as any) }));
      }
  ...
  const isImage = (m: MediaItem) => m.kind === "file" && (m.mime || "").startsWith("image/");
  const isYoutube = (m: MediaItem) => m.kind === "external" && m.provider === "youtube";
  ...
  const confirmSelect = async () => {
    ...
    const selectedItems = ids.map((id) => selectedMap[id]).filter(Boolean) as MediaItem[];
  ...
  const doDelete = async () => {
    ...
    try {
      await axios.post(route("media.destroy", deleteId), { _method: "DELETE" }, { headers: { Accept: "application/json" } });
        ...
        <div className="col-12 col-lg-8">
          <div className="card h-100">
            <div className="card-header d-flex flex-wrap gap-2 align-items-center justify-content-between">
              <form className="d-flex flex-wrap gap-2 align-items-center" onSubmit={applySearch}>
                <div className="row g-3">
              ...
              {selectable && mode === "modal" && multiple && (
                <button className="btn btn-success" type="button" disabled={selected.length === 0} onClick={confirmSelect}>
            ...
            <div className="card-body">
              {selectable && mode === "modal" && (
                <div className="text-muted small mb-2">
                  Click to use one item. Hold <b>Ctrl</b>/<b>Cmd</b> and click to multi-select.
                </div>
              )}
              ...
              <div className="row g-3">
                {items.map((m) => {
                  ...
                  const label = m.title || m.original_name || (isYoutube(m) ? "YouTube" : `Media #${m.id}`);
                  const active = selectable && selected.includes(m.id);

                  return (
                    <div className="col-6 col-md-4 col-xl-3" key={m.id}>
                      <div
                        className={`border rounded h-100 p-2 ${active ? "border-primary" : ""}`}
                        style={{ cursor: selectable ? "pointer" : "default" }}
                        onClick={(e) => handleCardClick(e, m)}
                        title={selectable ? "Select / Use" : "Open"}
                      >
                        <div className="bg-light rounded d-flex align-items-center justify-content-center" style={{ height: 120, overflow: "hidden" }}>
                          {thumb ? (
                            <img src={thumb} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                          ) : (
                            <div className="text-muted small text-center px-2">{isDoc(m) ? "Document" : "Media"}</div>
                          )}
                        </div>

                        <div className="mt-2 small text-truncate" title={label}>
                          {label}
                        </div>

                        <div className="text-muted" style={{ fontSize: 12 }}>
                          {isYoutube(m) ? "YouTube" : m.mime || ""}
              ...
              {meta && meta.last_page > 1 && (
                <div className="d-flex justify-content-center gap-2 mt-4">
                  <button className="btn btn-outline-secondary" disabled={page <= 1} onClick={() => load(page - 1)}>
                    Prev
                  </button>
                  <div className="align-self-center">
                    Page {meta.current_page} / {meta.last_page}
                  </div>
                  <button className="btn btn-outline-secondary" disabled={page >= meta.last_page} onClick={() => load(page + 1)}>
                    Next
                  </button>
                </div>
              )}
          ...
          <div className="col-12 col-lg-4">
            <div className="card mb-3">
              <div className="card-header">
                ...
                <div className="text-muted small">Select files, review, then click Upload.</div>
              </div>
              ...
              <div className="card-body">
                {mode === "modal" && (
                  <div className="form-check form-switch mb-2">
                    <input
                      ...
                      onChange={(e) => onUseAfterUploadChange?.(e.target.checked)}
                      disabled={uploading}
                    />
                    <label className="form-check-label" htmlFor="useAfterUploadSwitch">
                      Use after upload
                    </label>
                  </div>
                )}
                ...
                <div className="d-flex gap-2 flex-wrap mb-3">
                  ...
                  <button className="btn btn-primary" type="button" disabled={uploading || queue.length === 0} onClick={uploadQueue}>
                    {uploading ? `Uploading ${progress}%...` : `Upload (${queue.length})`}
                  </button>

                  <button className="btn btn-outline-secondary" type="button" disabled={uploading || queue.length === 0} onClick={clearQueue}>
                    Clear
                  </button>
                </div>

                {uploadError && <div className="alert alert-danger py-2 mb-2">{uploadError}</div>}
                          ...
                          return (
                            <tr key={key}>
                              ...
                              <td style={{ minWidth: 0 }}>
                                ...
                                <div className="text-muted small text-truncate">{f.type || "unknown"}</div>
                              </td>
                              <td className="text-end">
                                <button className="btn btn-sm btn-outline-danger" type="button" disabled={uploading} onClick={() => removeFromQueue(key)}>
                                  Remove
                                </button>
                              </td>
                            </tr>
            ...
            <div className="card">
              <div className="card-header">
                ...
                <div className="text-muted small">Save a YouTube link into Media Bank.</div>
              </div>

              <div className="card-body">
                <div className="mb-2">
                  <input className="form-control" placeholder="YouTube URL" value={ytUrl} onChange={(e) => setYtUrl(e.target.value)} />
                </div>
                <div className="mb-2">
                  <input className="form-control" placeholder="Title (optional)" value={ytTitle} onChange={(e) => setYtTitle(e.target.value)} />
                </div>
                <button className="btn btn-outline-success w-100" type="button" onClick={addYoutube} disabled={ytBusy}>
      ...
      <ConfirmModal
        ...
        message={
          <>
            ...
            {deleteError && <div className="alert alert-danger py-2 mt-3 mb-0">{deleteError}</div>}
          </>
        }
        confirmText="Delete"
