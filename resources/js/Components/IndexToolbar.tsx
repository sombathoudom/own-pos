...
export default function IndexToolbar({
  ...
  const reset = () => {
    ...
    setDirection("asc");
    setPerPage(10);
    router.get(route(indexRouteName), {}, { preserveScroll: true, preserveState: true, replace: true });
  };
  ...
  return (
    <form onSubmit={submit} className="row g-2 align-items-end mb-3">
      <div className="col-12 col-md-6">
        ...
        <input className="form-control" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={searchPlaceholder} />
      </div>

      <div className="col-6 col-md-3">
        <label className="form-label mb-1">Order</label>
        <select className="form-select" value={direction} onChange={(e) => setDirection(e.target.value as "asc" | "desc")}>
      ...
      <div className="col-12 d-flex justify-content-end">
        <button type="button" className="btn btn-link text-decoration-none" onClick={reset}>
          Reset
        </button>
      </div>
    </form>
